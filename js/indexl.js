var S = {
  init: function () {
    ...
    if (i !== -1) {
      ...
      // 2. Đợi 5.5s (đếm ngược xong) thì chạy chữ
      setTimeout(() => {
        // Tham số thứ 2 là hàm sẽ chạy SAU KHI chữ chạy xong
        S.UI.simulate('Hello!||HA MINH||HAPPY|NEW|YEAR||', function() {
...
S.UI = (function () {
  ...
  // CẬP NHẬT: Thêm tham số callback để biết khi nào chạy xong
  function timedAction(fn, delay, max, reverse, callback) {
    ...
    if (!max || (!reverse && currentAction < max) || (reverse && currentAction > 0)) {
      interval = setInterval(function () {
        ...
        if ((!reverse && max && currentAction === max) || (reverse && currentAction === 0)) {
          clearInterval(interval);
          // KHI CHẠY XONG -> GỌI CALLBACK
          if (callback) callback();
        }
      }, delay);
    } else {
      // Trường hợp chuỗi chỉ có 1 từ
      if (callback) callback();
    }
  }
  ...
  // CẬP NHẬT: Nhận callback từ simulate
  function performAction(value, callback) {
    ...
    }, 3500, sequence.length, false, callback); // Truyền callback vào đây
  }
  ...
  return {
    // CẬP NHẬT: Hàm simulate nhận thêm callback
